export const categoryList = [
  {
    id: "main1",
    name: "Language",
    url: "/language",
    sub: [
      {
        id: "main1-1",
        name: "Javascript",
        url: "/language/javascript",
        sub: [
          {
            id: 1,
            name: "basic",
            url: "/language/javascript/basic",
            list: [
              {
                id: 1,
                name: "자바스크립트",
                url: "/language/javascript/basic/1",
              },
              {
                id: 2,
                name: "변수와 연산자",
                url: "/language/javascript/basic/2",
              },
              {
                id: 3,
                name: "참조타입",
                url: "/language/javascript/basic/3",
              },
              {
                id: 4,
                name: "eval",
                url: "/language/javascript/basic/4",
              },
              {
                id: 5,
                name: "조건문과 반복문",
                url: "/language/javascript/basic/5",
              },
              {
                id: 6,
                name: "자료구조",
                url: "/language/javascript/basic/6",
              },
              {
                id: 7,
                name: "함수",
                url: "/language/javascript/basic/7",
              },
              {
                id: 8,
                name: "객체와 메서드",
                url: "/language/javascript/basic/8",
              },
              {
                id: 9,
                name: "프로퍼티 플래그와 프로토타입",
                url: "/language/javascript/basic/9",
              },
              {
                id: 10,
                name: "클래스",
                url: "/language/javascript/basic/10",
              },
              {
                id: 11,
                name: "JSON",
                url: "/language/javascript/basic/11",
              },
              {
                id: 12,
                name: "try / catch",
                url: "/language/javascript/basic/12",
              },
              {
                id: 13,
                name: "콜백, 프로미스",
                url: "/language/javascript/basic/13",
              },
              {
                id: 14,
                name: "제너레이터",
                url: "/language/javascript/basic/14",
              },
              {
                id: 15,
                name: "모듈",
                url: "/language/javascript/basic/15",
              },
              {
                id: 16,
                name: "Proxy와 Reflect",
                url: "/language/javascript/basic/16",
              },
            ],
          },
          {
            id: 2,
            name: "event / interface",
            url: "/language/javascript/event&interface",
            list: [
              {
                id: 1,
                name: "브라우저 환경과 스크립트 동작 방식",
                url: "/language/javascript/event&interface/1",
              },
              {
                id: 2,
                name: "DOM",
                url: "/language/javascript/event&interface/2",
              },
              {
                id: 3,
                name: "DOM Element 조작 및 스타일",
                url: "/language/javascript/event&interface/3",
              },
              {
                id: 4,
                name: "Observer",
                url: "/language/javascript/event&interface/4",
              },
              {
                id: 5,
                name: "Element 사이즈와 좌표",
                url: "/language/javascript/event&interface/5",
              },
              {
                id: 6,
                name: "이벤트",
                url: "/language/javascript/event&interface/6",
              },
              {
                id: 7,
                name: "UI 이벤트",
                url: "/language/javascript/event&interface/7",
              },
              {
                id: 8,
                name: "폼 입력 이벤트",
                url: "/language/javascript/event&interface/8",
              },
              {
                id: 9,
                name: "Selection / Range ",
                url: "/language/javascript/event&interface/9",
              },
              {
                id: 10,
                name: "이벤트 루프와 매크로태스크",
                url: "/language/javascript/event&interface/10",
              },
              {
                id: 11,
                name: "팝업",
                url: "/language/javascript/event&interface/11",
              },
            ],
          },
          {
            id: 3,
            name: "file",
            url: "/language/javascript/file",
            list: [
              {
                id: 1,
                name: "ArrayBuffer",
                url: "/language/javascript/file/1",
              },
              {
                id: 2,
                name: "TextDecoder / Encoder",
                url: "/language/javascript/file/2",
              },
              {
                id: 3,
                name: "Blob",
                url: "/language/javascript/file/3",
              },
              {
                id: 4,
                name: "File / FileReader",
                url: "/language/javascript/file/4",
              },
              {
                id: 5,
                name: "FormData",
                url: "/language/javascript/file/5",
              },
              {
                id: 6,
                name: "URL",
                url: "/language/javascript/file/6",
              },
              {
                id: 7,
                name: "XMLHttpRequest",
                url: "/language/javascript/file/7",
              },
            ],
          },
          {
            id: 4,
            name: "network",
            url: "/language/javascript/network",
            list: [
              {
                id: 1,
                name: "Fetch",
                url: "/language/javascript/network/1",
              },
              {
                id: 2,
                name: "CORS",
                url: "/language/javascript/network/2",
              },
              {
                id: 3,
                name: "Iframe",
                url: "/language/javascript/network/3",
              },
              {
                id: 4,
                name: "Polling",
                url: "/language/javascript/network/4",
              },
              {
                id: 5,
                name: "Web Socket",
                url: "/language/javascript/network/5",
              },
              {
                id: 6,
                name: "Server Sent Events",
                url: "/language/javascript/network/6",
              },
            ],
          },
          {
            id: 5,
            name: "browser data",
            url: "/language/javascript/browser_data",
            list: [
              {
                id: 1,
                name: "Cookie",
                url: "/language/javascript/browser_data/1",
              },
              {
                id: 2,
                name: "Browser Storage",
                url: "/language/javascript/browser_data/2",
              },
              {
                id: 3,
                name: "IndexedDB",
                url: "/language/javascript/browser_data/3",
              },
            ],
          },
          {
            id: 6,
            name: "web components",
            url: "/language/javascript/webcomponents",
            list: [
              {
                id: 1,
                name: "Custom Element",
                url: "/language/javascript/webcomponents/1",
              },
              {
                id: 2,
                name: "Template",
                url: "/language/javascript/webcomponents/2",
              },
              {
                id: 3,
                name: "Shadow DOM",
                url: "/language/javascript/webcomponents/3",
              },
              {
                id: 4,
                name: "Shadow DOM Slot",
                url: "/language/javascript/webcomponents/4",
              },
              {
                id: 5,
                name: "Shadow DOM Style",
                url: "/language/javascript/webcomponents/5",
              },
              {
                id: 6,
                name: "Shadow DOM Event",
                url: "/language/javascript/webcomponents/6",
              },
            ],
          },
          {
            id: 7,
            name: "regular expression",
            url: "/language/javascript/regular_expression",
            list: [
              {
                id: 1,
                name: "패턴과 플래그",
                url: "/language/javascript/regular_expression/1",
              },
            ],
          },
        ],
      },
      { id: "main1-2", name: "JAVA", url: "/language/java", sub: [] },
      { id: "main1-3", name: "Python", url: "/language/python", sub: [] },
      {
        id: "main1-4",
        name: "PHP",
        url: "/language/php",
        sub: [
          {
            id: 1,
            name: "basic",
            url: "/language/php/basic",
            list: [
              {
                id: 1,
                name: "php",
                url: "/language/php/basic/1",
              },
              {
                id: 2,
                name: "변수와 상수",
                url: "/language/php/basic/2",
              },
              {
                id: 3,
                name: "타입",
                url: "/language/php/basic/3",
              },
              {
                id: 4,
                name: "표현식과 연산자",
                url: "/language/php/basic/4",
              },
              {
                id: 5,
                name: "제어문",
                url: "/language/php/basic/5",
              },
              {
                id: 6,
                name: "함수",
                url: "/language/php/basic/6",
              },
              {
                id: 7,
                name: "자주 쓰이는 내장 함수",
                url: "/language/php/basic/7",
              },
              {
                id: 8,
                name: "클래스와 인터페이스",
                url: "/language/php/basic/8",
              },
              {
                id: 9,
                name: "Trait",
                url: "/language/php/basic/9",
              },
              {
                id: 10,
                name: "네임스페이스",
                url: "/language/php/basic/10",
              },
              {
                id: 11,
                name: "Enumerations",
                url: "/language/php/basic/11",
              },
              {
                id: 12,
                name: "에러 및 예외",
                url: "/language/php/basic/12",
              },
              {
                id: 13,
                name: "Fibers, Generators",
                url: "/language/php/basic/13",
              },
              {
                id: 14,
                name: "Attributes",
                url: "/language/php/basic/14",
              },
              {
                id: 15,
                name: "References",
                url: "/language/php/basic/15",
              },
            ],
          },
          {
            id: 2,
            name: "network",
            url: "/language/php/network",
            list: [
              {
                id: 1,
                name: "Cookie, HTTP",
                url: "/language/php/network/1",
              },
              {
                id: 2,
                name: "Session",
                url: "/language/php/network/2",
              },
            ],
          },
        ],
      },
      {
        id: "main1-5",
        name: "Dart",
        url: "/language/dart",
        sub: [
          {
            id: 1,
            name: "basic",
            url: "/language/dart/basic",
            list: [
              {
                id: 1,
                name: "dart",
                url: "/language/dart/basic/1",
              },
              {
                id: 2,
                name: "변수, 연산자",
                url: "/language/dart/basic/2",
              },
              {
                id: 3,
                name: "주석, 메타데이터",
                url: "/language/dart/basic/3",
              },
              {
                id: 4,
                name: "라이브러리, 임포트",
                url: "/language/dart/basic/4",
              },
              {
                id: 5,
                name: "타입",
                url: "/language/dart/basic/5",
              },
              {
                id: 6,
                name: "패턴 및 제어문",
                url: "/language/dart/basic/6",
              },
              {
                id: 7,
                name: "함수",
                url: "/language/dart/basic/7",
              },
              {
                id: 8,
                name: "예외 처리",
                url: "/language/dart/basic/8",
              },
              {
                id: 9,
                name: "클래스와 객체",
                url: "/language/dart/basic/9",
              },
              {
                id: 10,
                name: "비동기 프로그래밍",
                url: "/language/dart/basic/10",
              },
            ],
          },
          {
            id: 2,
            name: "library",
            url: "/language/dart/library",
            list: [
              {
                id: 1,
                name: "core",
                url: "/language/dart/library/1",
              },
              {
                id: 2,
                name: "async",
                url: "/language/dart/library/2",
              },
              {
                id: 3,
                name: "math",
                url: "/language/dart/library/3",
              },
              {
                id: 4,
                name: "convert",
                url: "/language/dart/library/4",
              },
              {
                id: 5,
                name: "io",
                url: "/language/dart/library/5",
              },
            ],
          },
        ],
      },
    ],
  },
  {
    id: "main2",
    name: "Stack",
    url: "/stack",
    sub: [
      {
        id: "main2-1",
        name: "Web Framework",
        url: "/stack/framework",
        sub: [
          {
            id: "main2-1-1",
            name: "react",
            url: "/stack/framework/react",
            list: [],
          },
          {
            id: "main2-1-3",
            name: "nextjs",
            url: "/stack/framework/nextjs",
            list: [],
          },
          {
            id: "main2-1-2",
            name: "vue",
            url: "/stack/framework/vue",
            list: [
              {
                id: 1,
                name: "vue 시작하기",
                url: "/stack/framework/vue/1",
              },
              {
                id: 2,
                name: "state, method",
                url: "/stack/framework/vue/2",
              },
              {
                id: 3,
                name: "binding",
                url: "/stack/framework/vue/3",
              },
              {
                id: 4,
                name: "rendering",
                url: "/stack/framework/vue/4",
              },
              {
                id: 5,
                name: "event handling",
                url: "/stack/framework/vue/5",
              },
              {
                id: 6,
                name: "vue 생명 주기",
                url: "/stack/framework/vue/6",
              },
              {
                id: 7,
                name: "watch",
                url: "/stack/framework/vue/7",
              },
              {
                id: 8,
                name: "템플릿 참조",
                url: "/stack/framework/vue/8",
              },
              {
                id: 9,
                name: "컴포넌트",
                url: "/stack/framework/vue/9",
              },
              {
                id: 10,
                name: "컴포저블",
                url: "/stack/framework/vue/10",
              },
            ],
          },
          {
            id: "main2-1-4",
            name: "spring boot",
            url: "/stack/framework/spring_boot",
            list: [],
          },
          {
            id: "main2-1-5",
            name: "laravel",
            url: "/stack/framework/laravel",
            list: [
              {
                id: 1,
                name: "laravel 시작하기",
                url: "/stack/framework/laravel/1",
              },
              {
                id: 2,
                name: "concept",
                url: "/stack/framework/laravel/2",
              },
              {
                id: 3,
                name: "route",
                url: "/stack/framework/laravel/3",
              },
              {
                id: 4,
                name: "middleware",
                url: "/stack/framework/laravel/4",
              },
              {
                id: 5,
                name: "controller",
                url: "/stack/framework/laravel/5",
              },
              {
                id: 6,
                name: "request, response, session",
                url: "/stack/framework/laravel/6",
              },
              {
                id: 7,
                name: "event",
                url: "/stack/framework/laravel/7",
              },
              {
                id: 8,
                name: "view, blade template",
                url: "/stack/framework/laravel/8",
              },
              {
                id: 9,
                name: "validation",
                url: "/stack/framework/laravel/9",
              },
              {
                id: 10,
                name: "error handling",
                url: "/stack/framework/laravel/10",
              },
              {
                id: 11,
                name: "security",
                url: "/stack/framework/laravel/11",
              },
              {
                id: 12,
                name: "database",
                url: "/stack/framework/laravel/12",
              },
              {
                id: 13,
                name: "eloquent orm",
                url: "/stack/framework/laravel/13",
              },
            ],
          },
        ],
      },
      // {
      //   id: "main2-2",
      //   name: "Web Library",
      //   url: "/stack/web_library",
      //   sub: [
      //     {
      //       id: "main2-2-1",
      //       name: "styled-components",
      //       url: "/stack/library/styled-components",
      //       list: [],
      //     },
      //     {
      //       id: "main2-2-2",
      //       name: "tailwindcss",
      //       url: "/stack/library/tailwindcss",
      //       list: [],
      //     },
      //     {
      //       id: "main2-2-3",
      //       name: "react-query",
      //       url: "/stack/library/react-query",
      //       list: [],
      //     },
      //   ],
      // },
      {
        id: "main2-3",
        name: "App Framework",
        url: "/stack/app",
        sub: [
          {
            id: "main2-3-1",
            name: "flutter",
            url: "/stack/app/flutter",
            list: [
              {
                  id: 1,
                  name: "layout",
                  url: "/stack/app/flutter/1",
              },
              {
                  id: 2,
                  name: "테마 디자인 및 미디어",
                  url: "/stack/app/flutter/2",
              },
              {
                  id: 3,
                  name: "상태 및 상호 작용",
                  url: "/stack/app/flutter/3",
              },
              {
                id: 4,
                name: "라우트",
                url: "/stack/app/flutter/4",
              },
              {
                id: 5,
                name: "애니메이션",
                url: "/stack/app/flutter/5",
              },
              {
                id: 6,
                name: "http",
                url: "/stack/app/flutter/6",
              },
              {
                id: 7,
                name: "데이터 저장",
                url: "/stack/app/flutter/7",
              },
              {
                id: 8,
                name: "성능",
                url: "/stack/app/flutter/8",
              },
              {
                id: 9,
                name: "상태 관리",
                url: "/stack/app/flutter/9",
              },
            ],
          },
        ],
      },
    ],
  },
  {
    id: "main3",
    name: "Computer",
    url: "/computer",
    sub: [
      //   { id: "main3-1", name: "test1" },
      //   { id: "main3-2", name: "test2" },
    ],
  },
];
